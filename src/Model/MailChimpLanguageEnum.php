<?php

declare(strict_types=1);

namespace MangoSylius\MailChimpPlugin\Model;

class MailChimpLanguageEnum
{
	public const ENGLISH = 'en';
	public const ARABIC = 'ar';
	public const AFRIKAANS = 'af';
	public const BELARUSIAN = 'be';
	public const BULGARIAN = 'bg';
	public const CATALAN = 'ca';
	public const CHINESE = 'zh';
	public const CROATIAN = 'hr';
	public const CZECH = 'cs';
	public const DANISH = 'da';
	public const DUTCH = 'nl';
	public const ESTONIAN = 'et';
	public const FARSI = 'fa';
	public const FINNISH = 'fi';
	public const FRENCH_FRANCE = 'fr';
	public const FRENCH_CANADA = 'fr_CA';
	public const GERMAN = 'de';
	public const GREEK = 'el';
	public const HEBREW = 'he';
	public const HINDI = 'hi';
	public const HUNGARIAN = 'hu';
	public const ICELANDIC = 'is';
	public const INDONESIAN = 'id';
	public const IRISH = 'ga';
	public const ITALIAN = 'it';
	public const JAPANESE = 'ja';
	public const KHMER = 'km';
	public const KOREAN = 'ko';
	public const LATVIAN = 'lv';
	public const LITHUANIAN = 'lt';
	public const MALTESE = 'mt';
	public const MALAY = 'ms';
	public const MACEDONIAN = 'mk';
	public const NORWEGIAN = 'no';
	public const POLISH = 'pl';
	public const PORTUGUESE_BRAZIL = 'pt';
	public const PORTUGUESE_PORTUGAL = 'pt_PT';
	public const ROMANIAN = 'ro';
	public const RUSSIAN = 'ru';
	public const SERBIAN = 'sr';
	public const SLOVAK = 'sk';
	public const SLOVENIAN = 'sl';
	public const SPANISH_MEXICO = 'es';
	public const SPANISH_SPAIN = 'es_ES';
	public const SWAHILI = 'sw';
	public const SWEDISH = 'sv';
	public const TAMIL = 'ta';
	public const THAI = 'th';
	public const TURKISH = 'tr';
	public const UKRAINIAN = 'uk';
	public const VIETNAMESE = 'vi';

	public const SUPPORTED_LANGUAGES = [
		self::ENGLISH,
		self::ARABIC,
		self::AFRIKAANS,
		self::BELARUSIAN,
		self::BULGARIAN,
		self::CATALAN,
		self::CHINESE,
		self::CROATIAN,
		self::CZECH,
		self::DANISH,
		self::DUTCH,
		self::ESTONIAN,
		self::FARSI,
		self::FINNISH,
		self::FRENCH_FRANCE,
		self::FRENCH_CANADA,
		self::GERMAN,
		self::GREEK,
		self::HEBREW,
		self::HINDI,
		self::HUNGARIAN,
		self::ICELANDIC,
		self::INDONESIAN,
		self::IRISH,
		self::ITALIAN,
		self::JAPANESE,
		self::KHMER,
		self::KOREAN,
		self::LATVIAN,
		self::LITHUANIAN,
		self::MALTESE,
		self::MALAY,
		self::MACEDONIAN,
		self::NORWEGIAN,
		self::POLISH,
		self::PORTUGUESE_BRAZIL,
		self::PORTUGUESE_PORTUGAL,
		self::ROMANIAN,
		self::RUSSIAN,
		self::SERBIAN,
		self::SLOVAK,
		self::SLOVENIAN,
		self::SPANISH_MEXICO,
		self::SPANISH_SPAIN,
		self::SWAHILI,
		self::SWEDISH,
		self::TAMIL,
		self::THAI,
		self::TURKISH,
		self::UKRAINIAN,
		self::VIETNAMESE,
	];
}
